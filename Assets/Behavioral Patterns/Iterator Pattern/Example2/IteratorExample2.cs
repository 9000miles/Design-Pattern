...
{
    private int[] intArr = new int[] { 46, 8, 16, 6, 99 };
    ...
    private void Start()
    {
        ...
        dic.Add(49, "Joofowe");
        dic.Add(499, "ppkerpk");
        dic.Add(9879, "ooweoor");
        dic.Add(4987, "ojofjweof");
        Debug.Log("----------------------");
        List<IEnumerator> list = new List<IEnumerator>();
        list.Add(intArr.GetEnumerator());
        list.Add(listInt.GetEnumerator());
        list.Add(dic.GetEnumerator());
        foreach (IEnumerator item in list)
        {
            while (item.MoveNext())
            {
                //string info = (string)item.Current;
        ...
        //Debug.Log("----------------------");
        //List<IEnumerator> list = new List<IEnumerator>();
        //list.Add(song70s.GetIterator());
        //list.Add(song80s.GetIterator());
        //foreach (IEnumerator item in list)
        //{
        //    while (item.MoveNext())
        //    {
        //        SongInfo info = (SongInfo)item.Current;
        //        Debug.Log(/*"Song 80s: " +*/ info.ToStringEx());
        //    }
        //}
    }
